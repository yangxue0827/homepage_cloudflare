...
import { NTag, NBadge, NButton, NSwitch } from 'naive-ui';
...
const availableFilters = computed(() => {
    if (displayMode.value === 'year') {
        ...
        years.sort((a, b) => Number(b) - Number(a)); // Sort years in descending order
...
const filteredPosts = computed(() => {
    ...
    if (displayMode.value === 'year') {
        ...
        if (typeof(selectedFilter.value) === 'number') {
            ...
            return groupedPosts.value.filter(([year]) => year === selectedFilter.value);
        }
        ...
        return groupedPosts.value.filter(([tag]) => tag === selectedFilter.value);
    }
});
                ...
                <div v-for="post in Posts" :key="post.frontmatter.title" class="sec-pub-papers-item flex">
                    <div class="paper-tags min-w-16 max-w-16 mr-2 flex flex-col gap-2 p-1">
                        <n-tag 
                            ...
                            strong 
                            size="small" 
                            class="w-full justify-center overflow-hidden hover:w-max"
                        >
...
.active-tag.active {
    @apply text-[#1661AB] font-semibold
}
.active-tag{
